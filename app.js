...
const PLAYER_SETTINGS_KEY = 'zedstream_player_settings';
...
function savePlayerSettings(settings) {
  try {
    const existing = loadPlayerSettings();
    const merged = { ...existing, ...settings, _savedAt: Date.now() };
    localStorage.setItem(PLAYER_SETTINGS_KEY, JSON.stringify(merged));
  } catch(e) { console.warn('[Settings] Save failed:', e); }
}

function getPlayerSetting(key, defaultValue) {
  const settings = loadPlayerSettings();
  return settings[key] !== undefined ? settings[key] : defaultValue;
}

/* ===== END PLAYER SETTINGS PERSISTENCE ===== */
...
/* ===== VIDEO FIT MODE SYSTEM ===== */
let currentFitMode = 'contain';

const FIT_MODES = [
  { id: 'contain', label: 'Contain', desc: 'Fit with letterbox bars', icon: 'contain' },
  { id: 'cover', label: 'Cover', desc: 'Fill screen, crop edges', icon: 'cover' },
  { id: 'fill', label: 'Fill', desc: 'Stretch to fill', icon: 'fill' },
  { id: 'none', label: 'Original', desc: 'Native resolution', icon: 'none' }
];
...
const playerFitBtn = document.getElementById('playerFitBtn');
...
async function fetchSubtitleContent(subUrl) {
  ...
  // Try direct fetch first (works if subtitle server has CORS headers)
  try {
    const resp = await fetchWithTimeout(subUrl, TIMEOUT_MS);
    if (resp.ok) {
      const text = await resp.text();
      return { content: text };
    }
  } catch (e) {
    console.log('[Subtitles] Direct fetch failed:', e.message);
  }

  // Fallback 1: allorigins.win (most reliable public proxy)
  try {
    const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(subUrl)}`;
    ...
    console.log('[Subtitles] allorigins proxy failed:', e.message);
  }

  // Fallback 2: thingproxy (alternative public proxy)
  try {
    const proxyUrl = `https://thingproxy.freeboard.io/fetch/${subUrl}`;
    const resp = await fetchWithTimeout(proxyUrl, TIMEOUT_MS);
    if (resp.ok) {
      const text = await resp.text();
      return { content: text };
    ...
    console.log('[Subtitles] thingproxy failed:', e.message);
  }

  // Fallback 3: corsproxy.io (last resort — often rate-limited)
  try {
    ...
    console.log('[Subtitles] corsproxy.io failed:', e.message);
  }

  return { error: 'All subtitle fetch attempts failed. The subtitle server may be unavailable or the file may not exist.' };
}
...
function srtToWebVTT(srtContent) {
  ...
  while (i < lines.length) {
    ...
    // Skip cue number (optional in SRT)
    if (/^\d+$/.test(line)) {
      i++;
      if (i >= lines.length) break;
    }

    // Match timing line: HH:MM:SS,mmm --> HH:MM:SS,mmm (with optional positioning)
    ...
    // Properly format: keep HH:MM:SS, replace comma with dot for milliseconds
    const startTime = timingMatch[1] + '.' + timingMatch[2];
    const endTime = timingMatch[3] + '.' + timingMatch[4];
    const positioning = timingMatch[5] ? ' ' + timingMatch[5].trim() : '';
    ...
    while (i < lines.length && lines[i].trim()) {
      let textLine = lines[i]
        ...
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;');
      cueText.push(textLine);
      i++;
    }

    if (cueText.length > 0) {
      // WebVTT: cue identifier is OPTIONAL — go straight to timing line
      vtt += `${startTime} --> ${endTime}${positioning}\n`;
      vtt += cueText.join('\n') + '\n\n';
    }
  }
...
function createSubtitleBlobUrl(content, format) {
  ...
  const isSrt = format === 'srt' || format === 'SRT' || 
  ...
  // Validate: must start with WEBVTT
  if (!vttContent.trim().startsWith('WEBVTT')) {
    console.error('[Subtitles] Invalid VTT content after conversion');
    // Try to fix by prepending WEBVTT header
    vttContent = 'WEBVTT\n\n' + vttContent;
  }

  // Create Blob with proper charset
...
function openPlayerUI(title, subtitle) {
  ...
  // Restore saved player settings before opening
  const savedSettings = loadPlayerSettings();
  if (savedSettings.playbackSpeed) currentPlaybackSpeed = savedSettings.playbackSpeed;
  if (savedSettings.subtitlePosition) currentSubtitlePosition = savedSettings.subtitlePosition;
  if (savedSettings.fitMode) currentFitMode = savedSettings.fitMode;
  if (savedSettings.lastVolume !== undefined) lastVolume = savedSettings.lastVolume;
  if (savedSettings.isMuted !== undefined) isMuted = savedSettings.isMuted;

  playerVideo.playbackRate = currentPlaybackSpeed;

  // Apply volume settings immediately
  playerVideo.volume = isMuted ? 0 : lastVolume;
...
playerVolumeBtn.addEventListener('click', (e) => {
  ...
  savePlayerSettings({ lastVolume: lastVolume, isMuted: isMuted });
  updateVolumeUI();
  showControls();
});
...
playerVolumeSlider.addEventListener('click', (e) => {
  ...
  lastVolume = pct;
  savePlayerSettings({ lastVolume: lastVolume, isMuted: false });
  updateVolumeUI();
});
...
function buildSpeedMenu() {
  ...
  playerSpeedDropdown.querySelectorAll('.speed-opt').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      savePlayerSettings({ playbackSpeed: s });
      buildSpeedMenu();
      closeSpeedDropdown();
      showControls();
    });
...
const POPULAR_SUBTITLE_LANGUAGES = new Set([
  'en', 'eng', 'english',
  'es', 'spa', 'spanish',
  'fr', 'fra', 'fre', 'french',
  'de', 'deu', 'ger', 'german',
...
function isPopularSubtitleLanguage(sub) {
  const rawLang = (sub.languageName || sub.language || sub.lang || '').toLowerCase().trim();
  if (!rawLang) return false;
  if (POPULAR_SUBTITLE_LANGUAGES.has(rawLang)) return true;
  // Also check if the resolved language name matches
  const resolved = getLanguageName(rawLang).toLowerCase();
  for (const popular of POPULAR_SUBTITLE_LANGUAGES) {
    if (resolved.includes(popular)) return true;
  }
  return false;
}

function isEnglishSubtitle(sub) {
  const rawLang = (sub.languageName || sub.language || sub.lang || '').toLowerCase().trim();
  return rawLang === 'en' || rawLang === 'eng' || rawLang === 'english' ||
         getLanguageName(rawLang).toLowerCase() === 'english';
}
...
/* Long-press or right-click subtitle button for language menu */
let subBtnPressTimer = null;
playerSubBtn.addEventListener('mousedown', (e) => {
  if (e.button !== 0) return; // Only left click
  subBtnPressTimer = setTimeout(() => {
    subBtnPressTimer = null;
    toggleSubDropdown();
    showControls();
  }, 500);
});
playerSubBtn.addEventListener('mouseup', () => {
  if (subBtnPressTimer) { clearTimeout(subBtnPressTimer); subBtnPressTimer = null; }
});
playerSubBtn.addEventListener('mouseleave', () => {
  if (subBtnPressTimer) { clearTimeout(subBtnPressTimer); subBtnPressTimer = null; }
});

playerSubPosBtn.addEventListener('click', (e) => {
  e.stopPropagation();
  cycleSubtitlePosition();
  showControls();
});

/* Fit Mode button — cycles through modes */
playerFitBtn.addEventListener('click', (e) => {
  e.stopPropagation();
  cycleFitMode();
  showControls();
});
...
/* ===== VIDEO FIT MODE — SINGLE BUTTON CYCLING ===== */

function initFitMode() {
  const savedFit = getPlayerSetting('fitMode', localStorage.getItem('zedstream_fit_mode'));
  if (savedFit && FIT_MODES.some(f => f.id === savedFit)) {
...
function applyFitMode(mode) {
  ...
  FIT_MODES.forEach(f => wrapper.classList.remove('fit-' + f.id));
  wrapper.classList.add('fit-' + mode);
  wrapper.style.setProperty('--video-fit', mode);

  currentFitMode = mode;
  localStorage.setItem('zedstream_fit_mode', mode);
  savePlayerSettings({ fitMode: mode });

  // Update button icon
  updateFitButtonIcon(mode);

  // Show brief indicator
  showFitIndicator(mode);

  console.log('[FitMode] Applied:', mode);
}

function updateFitButtonIcon(mode) {
  // Hide all icons
  const icons = ['fitIconContain', 'fitIconCover', 'fitIconFill', 'fitIconNone'];
  icons.forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = 'none';
  });
  // Show active icon
  const activeMap = { contain: 'fitIconContain', cover: 'fitIconCover', fill: 'fitIconFill', none: 'fitIconNone' };
  const activeId = activeMap[mode];
  if (activeId) {
...
function showFitIndicator(mode) {
  const existing = document.querySelector('.player-fit-indicator');
  if (existing) existing.remove();

  const indicator = document.createElement('div');
  indicator.className = 'player-fit-indicator visible';
  const modeLabel = FIT_MODES.find(f => f.id === mode)?.label || mode;
  indicator.innerHTML = '<span class="fit-badge">Fit</span>' + modeLabel;

  const videoArea = document.getElementById('playerVideoArea');
  if (videoArea) videoArea.appendChild(indicator);
...
function cycleFitMode() {
  const fitCycle = ['contain', 'cover', 'fill', 'none'];
  const currentIdx = fitCycle.indexOf(currentFitMode);
  const nextFit = fitCycle[(currentIdx + 1) % fitCycle.length];
  applyFitMode(nextFit);
  const label = FIT_MODES.find(f => f.id === nextFit)?.label || nextFit;
  showToast('Video Fit', `Switched to ${label}`, 'info');
}

function initSubtitlePosition() {
  // Load saved position from unified settings (fallback to legacy localStorage)
  const saved = getPlayerSetting('subtitlePosition', localStorage.getItem('zedstream_subtitle_position'));
  if (saved && SUBTITLE_POSITIONS.some(p => p.id === saved)) {
    currentSubtitlePosition = saved;
  } else {
    currentSubtitlePosition = 'bottom'; // Default to bottom for new users
  }
  applySubtitlePosition(currentSubtitlePosition);

  // Load saved subtitle size
  const savedSize = getPlayerSetting('subtitleSize', localStorage.getItem('zedstream_subtitle_size') || 'medium');
  applySubtitleSize(savedSize);

  // Auto-refresh subtitle position on resize (but don't force any position)
  let resizeTimer = null;
  window.addEventListener('resize', () => {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(() => {
      // Re-apply current position to handle any CSS recalculation needs
      applySubtitlePosition(currentSubtitlePosition);
    }, 200);
  });
  ...
  document.addEventListener('fullscreenchange', () => {
    // Re-apply current position after fullscreen transition
    setTimeout(() => {
      applySubtitlePosition(currentSubtitlePosition);
    }, 100);
  });
...
function applySubtitleSize(size) {
  ...
  sizes.forEach(s => wrapper.classList.remove('subtitle-size-' + s));
  wrapper.classList.add('subtitle-size-' + size);
  localStorage.setItem('zedstream_subtitle_size', size);
  savePlayerSettings({ subtitleSize: size });
  console.log('[Subtitles] Size set to:', size);
}

function cycleSubtitleSize() {
  const sizes = ['small', 'medium', 'large', 'xl'];
  const current = localStorage.getItem('zedstream_subtitle_size') || 'medium';
  const idx = sizes.indexOf(current);
  const next = sizes[(idx + 1) % sizes.length];
  applySubtitleSize(next);
  const labels = { small: 'Small', medium: 'Medium', large: 'Large', xl: 'Extra Large' };
  showToast('Subtitle Size', `Changed to: ${labels[next]}`, 'info');
}
...
function applySubtitlePosition(position) {
  ...
  // Remove all position classes with smooth transition
  ...
  // Save preference always (landscape or portrait)
  localStorage.setItem('zedstream_subtitle_position', position);
  savePlayerSettings({ subtitlePosition: position });
  currentSubtitlePosition = position;

  // Force a subtitle re-render to apply new positioning immediately
  setTimeout(() => {
...
function cycleSubtitlePosition() {
  const posCycle = ['bottom', 'middle', 'top', 'lower-third', 'very-bottom'];
  const currentIdx = posCycle.indexOf(currentSubtitlePosition);
  const nextPos = posCycle[(currentIdx + 1) % posCycle.length];
  applySubtitlePosition(nextPos);
  const label = SUBTITLE_POSITIONS.find(p => p.id === nextPos)?.label || nextPos;
  showToast('Subtitle Position', `Switched to: ${label}`, 'info');

  // CRITICAL FIX: Force subtitle refresh after position change
  setTimeout(() => {
    forceSubtitleRefresh();
  }, 100);
}

function updateSubPosButtonIcon(position) {
  const icons = ['subposIconBottom', 'subposIconTop', 'subposIconMiddle', 'subposIconLowerThird', 'subposIconVeryBottom'];
  icons.forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = 'none';
  });
  const activeMap = {
    bottom: 'subposIconBottom',
    top: 'subposIconTop',
    middle: 'subposIconMiddle',
    'lower-third': 'subposIconLowerThird',
    'very-bottom': 'subposIconVeryBottom'
  };
  const activeId = activeMap[position];
  if (activeId) {
    const el = document.getElementById(activeId);
    if (el) el.style.display = 'block';
  }
...
function toggleSubtitles() {
  // Toggle subtitles ON/OFF (not the dropdown menu)
  if (subtitlesEnabled && selectedSubtitleUrl) {
    // Currently ON -> turn OFF
    disableSubtitles();
    subtitlesEnabled = false;
      playerSubBtn.classList.remove('active');
  subtitlesEnabled = false;
  selectedSubtitleUrl = null;   playerTopSubtitle.textContent = 'Subtitles: Off';
    showToast('Subtitles', 'Turned off', 'info');
  } else {
    // Currently OFF -> turn ON (auto-load best available)
    subtitlesEnabled = true;
    playerSubBtn.classList.add('active');
    if (currentSubtitleTracks && currentSubtitleTracks.length > 0) {
      autoLoadEnglishSubtitle();
    } else {
      playerTopSubtitle.textContent = 'Subtitles: No tracks available';
      showToast('Subtitles', 'No subtitle tracks available for this title', 'error');
    }
  }
}

function toggleSubDropdown() {
  // Only toggles the language selection dropdown (does NOT turn subtitles on/off)
...
function closeSubDropdown() {
  ...
  // DON'T remove active class here - that would turn off subtitles!
  // Only close the menu, keep subtitle state as-is
  clearTimeout(controlsHideTimer);
  controlsHideTimer = setTimeout(hideControls, 3000);
}
...
function buildSubtitleMenu() {
  ...
  if (!currentSubtitleTracks || currentSubtitleTracks.length === 0) {
    playerSubDropdown.innerHTML = '<div class="sub-opt" style="cursor:default;opacity:0.5;padding:12px 16px;">No subtitles available</div>';
    return;
  }

  // DEDUPLICATE: Keep only ONE subtitle per language (best quality first)
  const langMap = new Map();
  currentSubtitleTracks.forEach(sub => {
    const rawLang = (sub.languageName || sub.language || sub.lang || 'unknown').toLowerCase().trim();
    const baseLang = rawLang.split('-')[0]; // "en-US" -> "en"
    const resolvedName = getLanguageName(rawLang);

    // Quality scoring: prefer hearing-impaired, then by format (vtt > srt), then by source reliability
    let score = 0;
    const label = (sub.label || '').toLowerCase();
    if (label.includes('sdh') || label.includes('hi') || label.includes('cc')) score += 100;
    const fmt = (sub.format || 'srt').toLowerCase();
    if (fmt === 'vtt' || fmt === 'webvtt') score += 50;
    if (fmt === 'srt') score += 25;
    // Prefer URLs that look like direct downloads over API wrappers
    const url = (sub.url || '');
    if (url.includes('.vtt') || url.includes('.srt')) score += 10;

    const existing = langMap.get(baseLang);
    if (!existing || score > existing._score) {
      langMap.set(baseLang, { ...sub, _score: score, _displayName: resolvedName });
    }
  });

  // Convert to array and sort: English first, then alphabetically
  const uniqueSubs = Array.from(langMap.values()).sort((a, b) => {
    const aIsEn = isEnglishSubtitle(a);
    const bIsEn = isEnglishSubtitle(b);
    if (aIsEn && !bIsEn) return -1;
    if (!aIsEn && bIsEn) return 1;
    return a._displayName.localeCompare(b._displayName);
  });

  // Add section header for English
  const hasEnglish = uniqueSubs.some(isEnglishSubtitle);
  if (hasEnglish) {
    const engHeader = document.createElement('div');
    engHeader.className = 'quality-group-label';
    engHeader.textContent = 'English';
    engHeader.style.cssText = 'padding: 10px 18px 6px; font-size: 11px; color: var(--accent-hover); text-transform: uppercase; letter-spacing: 1px; font-weight: 700;';
    playerSubDropdown.appendChild(engHeader);
  }

  let inEnglishSection = hasEnglish;

  uniqueSubs.forEach((sub) => {
    // Add "Other Languages" divider
    if (inEnglishSection && !isEnglishSubtitle(sub)) {
      inEnglishSection = false;
      const otherHeader = document.createElement('div');
      otherHeader.className = 'quality-group-label';
      otherHeader.textContent = 'Other Languages';
      otherHeader.style.cssText = 'padding: 10px 18px 6px; font-size: 11px; color: var(--text-quaternary); text-transform: uppercase; letter-spacing: 1px; font-weight: 700;';
      playerSubDropdown.appendChild(otherHeader);
    }
    ...
    const langName = sub._displayName;
    const isEng = isEnglishSubtitle(sub);
    ...
    opt.addEventListener('click', (e) => {
      e.stopPropagation();
      loadSubtitle(sub.url, sub.language || sub.lang || 'en');
      buildSubtitleMenu();
      closeSubDropdown();
      showControls();
    });
...
async function loadSubtitle(url, lang) {
  if (!url) {
    console.warn('[Subtitles] No URL provided');
    return;
  }
  ...
  // Step 1: Clean up ALL existing tracks thoroughly
  disableSubtitles();
  ...
  if (result.error) {
    ...
    playerSubBtn.classList.remove('active');
    playerTopSubtitle.textContent = 'Subtitle unavailable';
    selectedSubtitleUrl = null;
    subtitlesEnabled = false;
    return;
  }

  // Step 3: Detect format and convert
  const lowerUrl = url.toLowerCase();
  const format = lowerUrl.endsWith('.srt') ? 'srt' : 
                 lowerUrl.endsWith('.vtt') ? 'vtt' : 'srt';

  // Step 4: Create blob URL
  let blobUrl;
  try {
    blobUrl = createSubtitleBlobUrl(result.content, format);
  } catch (e) {
    console.error('[Subtitles] Blob creation failed:', e);
    showToast('Subtitle Error', 'Failed to process subtitle file', 'error');
    playerSubBtn.classList.remove('active');
    return;
  }

  // Step 5: Create track element
  const track = document.createElement('track');
  track.kind = 'subtitles';
  track.label = getLanguageName(lang) || 'Subtitle';
  track.srclang = (lang || 'en').toLowerCase().split('-')[0];
  track.src = blobUrl;
  track.default = true;

  // CRITICAL FIX: Wait for track to fully load before appending and enabling
  return new Promise((resolve, reject) => {
    ...
    track.addEventListener('load', () => {
      ...
      // Now safe to append and enable
      playerVideo.appendChild(track);

      // Find and enable the text track
      const textTracks = playerVideo.textTracks;
      for (let i = textTracks.length - 1; i >= 0; i--) {
        const tt = textTracks[i];
        if (tt.kind === 'subtitles' || tt.kind === 'captions') {
          // Must set mode AFTER track is loaded
          tt.mode = 'showing';
          console.log('[Subtitles] Track enabled:', track.label);
          break;
        }
      }
      ...
      savePlayerSettings({ selectedSubtitleUrl: url, selectedSubtitleLang: lang });
      showToast('Subtitle', `Loaded: ${displayLang || 'Subtitle'}`, 'success');
      resolve();
    });
    ...
    track.addEventListener('error', () => {
      ...
      showToast('Subtitle Error', 'Could not display subtitle track', 'error');
      reject(new Error('Track load failed'));
    });

    // Append track to trigger load
    playerVideo.appendChild(track);
  });
}


/* ===== AUTO-LOAD ENGLISH SUBTITLE ===== */
async function autoLoadEnglishSubtitle() {
  if (!currentSubtitleTracks || currentSubtitleTracks.length === 0) {
    console.log('[AutoSub] No subtitle tracks available');
    return;
  }

  // Deduplicate and find best English subtitle
  const langMap = new Map();
  currentSubtitleTracks.forEach(sub => {
    const rawLang = (sub.languageName || sub.language || sub.lang || 'unknown').toLowerCase().trim();
    ...
    const existing = langMap.get(baseLang);
    if (!existing || score > existing._score) {
      langMap.set(baseLang, { ...sub, _score: score });
    }
  });

  const englishSub = langMap.get('en') || langMap.get('eng') || Array.from(langMap.values()).find(sub => isEnglishSubtitle(sub));
  if (!englishSub) {
    console.log('[AutoSub] No English subtitle found among', currentSubtitleTracks.length, 'raw tracks (deduplicated to', langMap.size, 'languages)');
    return;
  }

  console.log('[AutoSub] Found English subtitle, loading...', englishSub.url.substring(0, 60));

  // Wait for video to be ready before loading subtitles
  let attempts = 0;
  const maxAttempts = 20;
  while ((!playerVideo || playerVideo.readyState < 1) && attempts < maxAttempts) {
    await new Promise(r => setTimeout(r, 200));
    attempts++;
  }

  if (!playerVideo || playerVideo.readyState < 1) {
    console.warn('[AutoSub] Video not ready after waiting, aborting subtitle load');
    return;
  }

  try {
    await loadSubtitle(englishSub.url, englishSub.language || 'en');
    playerSubBtn.classList.add('active');
    console.log('[AutoSub] English subtitle loaded successfully');
  } catch (e) {
    console.error('[AutoSub] Failed to auto-load English subtitle:', e);
    playerSubBtn.classList.remove('active');
  }
}
...
function disableSubtitles() {
  subtitlesEnabled = false;
  selectedSubtitleUrl = null;
  savePlayerSettings({ selectedSubtitleUrl: null, selectedSubtitleLang: null });
  if (playerSubBtn) playerSubBtn.classList.remove('active');

  // Step 1: Disable all textTracks first
  if (playerVideo && playerVideo.textTracks) {
    for (let i = playerVideo.textTracks.length - 1; i >= 0; i--) {
  ...
  // Step 2: Remove DOM track elements and revoke blob URLs
  if (playerVideo) {
  ...
  console.log('[Subtitles] All tracks disabled and removed');
}

/* ===== SUBTITLE REFRESH HELPER ===== */
function forceSubtitleRefresh() {
  if (!playerVideo || !playerVideo.textTracks || playerVideo.textTracks.length === 0) return;

  for (let i = 0; i < playerVideo.textTracks.length; i++) {
    const tt = playerVideo.textTracks[i];
    if (tt.mode === 'showing') {
      // Toggle mode to force refresh
      tt.mode = 'hidden';
      setTimeout(() => { 
  ...
  // Also try updating the CSS class on the wrapper to trigger re-render
  const wrapper = document.getElementById('playerVideoWrapper');
  if (wrapper) {
    const currentClass = wrapper.className;
    wrapper.className = currentClass + ' force-refresh';
    setTimeout(() => { wrapper.className = currentClass; }, 10);
  }
}
...
async function loadEmbeddedSubtitles(subtitles) {
  ...
  // Find first English subtitle or first available
  const sub = subtitles.find(s => isEnglishSubtitle(s)) || subtitles[0];
  if (!sub || !sub.url) return;

  console.log('[SmartPlayback] Loading embedded subtitle:', sub.url.substring(0, 60));
  await loadSubtitle(sub.url, sub.lang || sub.language || 'en');
}
...
function loadVideoSource(url, type, subtitles, isFallback) {
  ...
  // CRITICAL FIX: Preserve crossorigin for subtitle CORS
  newVideo.crossOrigin = 'anonymous';
  newVideo.playsInline = true;
  newVideo.id = 'playerVideo';
  ...
  // ===== SUBTITLE LOADING HELPER — LOADS BOTH EMBEDDED + WYZIE SUBS =====
  const loadAllSubtitles = () => {
    // 1. Load embedded subtitles from the video API response
    if (subtitles && subtitles.length > 0) {
      console.log('[Subtitles] Loading', subtitles.length, 'embedded subtitle(s)');
      loadEmbeddedSubtitles(subtitles);
    }
    // 2. Auto-load English from wyzie API (external subtitle search)
    if (currentSubtitleTracks && currentSubtitleTracks.length > 0) {
      console.log('[Subtitles] Auto-loading English from', currentSubtitleTracks.length, 'wyzie track(s)');
      autoLoadEnglishSubtitle();
    }
  };
  ...
  if (isHls && Hls.isSupported()) {
    ...
    // CRITICAL FIX: Load subtitles after video is actually playing (not just manifest parsed)
    hls.on(Hls.Events.BUFFER_APPENDED, () => {
      if (!playerVideo.paused && playerVideo.currentTime > 0) {
    ...
    // CRITICAL FIX: Load subtitles after video is actually playing
    playerVideo.addEventListener('playing', () => {
      loadAllSubtitles();
    }, { once: true });
...
async function playMovie(item) {
  ...
  if (data && !data.error && data.sources && data.sources.length > 0) {
    ...
    setTimeout(() => autoLoadEnglishSubtitle(), 1000);
  } else {
...
function closePlayer() {
  ...
  // Clean up subtitle tracks before clearing innerHTML
  disableSubtitles();
  playerVideo.innerHTML = '';
...
async function performRegister(email, password) {
  ...
  try {
    ...
    setTimeout(() => { window.location.href = 'go:payment'; }, 2000);
    setLoginLoading(false);
    return true;
  } catch(error) {
